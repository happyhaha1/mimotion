...
- GitHub Actions 使用 UTC 时间，即**北京时间 - 8**。默认每天有 9 个候选，对应北京时间 `7、9、12、16、17、20、21:37、22:37、23:37`：
  ```yaml
  on:
    schedule:
      - cron: '17 1 * * *'
      ...
      - cron: '17 9 * * *'
      - cron: '17 12 * * *'
      - cron: '37 13 * * *' # 北京时间 21:37，主候选
      - cron: '37 14 * * *' # 北京时间 22:37，兜底候选
      - cron: '37 15 * * *' # 北京时间 23:37，最后兜底
      - cron: '17 23 * * *'
  ```
- 普通时段固定在 UTC 第 `17` 分钟触发，工作流启动后随机等待到当前小时的 `17-50` 分钟。
- 晚间主候选在北京时间 `21:37` 启动后，会等待到 `22:00-22:20` 执行；`22:37` 和 `23:37` 是兜底候选。如果任务延迟到午夜以后，则按实际时间正常计算步数，不强制使用 `max_step`。
- `CRON_HOURS` 变量和 `Random Cron` 工作流已不再使用；修改小时列表后提交到默认分支即可。
- GitHub Actions 可能因为资源排队延迟或丢弃而执行不准时，cron 不是严格准点定时器。
...
1. 默认每天有 9 个候选，由 `run.yml` 中的 cron 控制。普通时段在每小时第 17 分钟被唤醒，然后随机等待到第 17-50 分钟执行；晚间有北京时间 21:37、22:37、23:37 三个候选，主候选会等待到 22:00-22:20。若任务延迟到午夜以后，则按实际时间正常计算步数。随机等待不会修改 GitHub 的 schedule 配置。
