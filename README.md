...
- GitHub Actions 使用 UTC 时间，即**北京时间 - 8**。默认每天北京时间 `7、9、12、16、17、20、22、23` 点执行，对应：
  ...
  on:
    schedule:
      - cron: '17 1,4,8,9,12,14,15,23 * * *'
  ```
- 分钟固定为 `17`，工作流启动后会在当前小时的 `17-50` 分钟之间随机等待，因此不要再通过提交修改 cron 的方式随机化。
- UTC `14、15` 分别对应北京时间 `22、23` 点，专门保留晚间高步数时段的候选执行机会。
...
1. 默认每天运行 8 次，由 `run.yml` 中的 cron 控制小时。工作流在每个小时的第 17 分钟被唤醒，然后随机等待到第 17-50 分钟执行；其中北京时间 22、23 点各有一个候选时段。随机等待不会修改 GitHub 的 schedule 配置。
